...
class RouterConfig 
{
     ...
     * @psalm-return (TValue is array ? \Symfony\Config\GosWebSocket\Server\Router\ResourcesConfig : static)
     */
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('resources', $value)) {
            $this->_usedProperties['resources'] = true;
            $this->resources = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\GosWebSocket\Server\Router\ResourcesConfig($v) : $v, $value['resources']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['resources'])) {
            $output['resources'] = array_map(fn ($v) => $v instanceof \Symfony\Config\GosWebSocket\Server\Router\ResourcesConfig ? $v->toArray() : $v, $this->resources);
        }
